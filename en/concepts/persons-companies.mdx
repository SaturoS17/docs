...
| **Crypto wallets** per network | **1** (`422 wallet_limit_reached` on the second) | **Unlimited** (use `label` to tell them apart) |
...
| **KYC** (`POST /v1/kyc`) | **Person** screening (`customer.person`), bills `compliance_person` | **Company/KYB** screening (`customer.company`), bills `compliance_company` |
...
- Registration: `POST /v1/auth/register` with `type: "person"` (or your
  operator creates it).
- KYC: submit `customer.person` with your identity —
  [guide](/en/guides/kyc).
- Crypto: **one wallet per network** (TRON and ETH). Need more addresses?
  That is a company-account use case.
...
- KYB: submit `customer.company` with the corporate data —
  [guide](/en/guides/kyc).
- Crypto: **unlimited wallets** per network (one per branch, per product,
  per vendor…), with a descriptive `label`.
...
| `422 wallet_limit_reached` | A person tried their second wallet on the same network |
