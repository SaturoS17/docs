...
**USDT is the operating currency**: fiat payouts and payins, cards and all
service fees always settle against the USDT balance. The other balances
move via [internal transfers](/en/guides/transfers) (always between
balances of the same currency), on-chain deposits (USDC) or credits from
your operator (BTC and GOLD).
...
by convention). It is display-only for valuing your balances — it implies
no conversion and no spread:
