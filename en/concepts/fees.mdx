...
| `wallet_import` | Fixed per import | When importing an external wallet into a [segregated wallet](/en/guides/segregated-wallets) (`POST /v1/wallets/import`) |
| `wallet_export` | Fixed per export | When exporting a segregated wallet's private key (`POST /v1/wallets/{id}/export`) |
| `wallet_send` | Fixed per send | When sending on-chain from a segregated wallet (`POST /v1/wallets/{id}/sends`); network gas is on the client |
