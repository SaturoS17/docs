...
Sign with a [segregated wallet](/en/guides/segregated-wallets) custodied by
...
<Steps>
  <Step title="Create the OTP challenge">
    Ask for a one-time code for the `sign_message` action and verify it to get
    an `X-OTP-Token` (see [OTP](/en/security-2fa)).
  </Step>
...
```
  ...
  <Step title="Sign the envelope in the wallet">
...
Subscribe to these events (see [Webhooks](/en/webhooks)):
...
<AccordionGroup>
  <Accordion title="Does a signature proof move funds or cost gas?">
    No. Signing a message is purely cryptographic: no on-chain transaction, no
    network fee, no balance change.
  </Accordion>
  <Accordion title="How long is a proof valid?">
  ...
  <Accordion title="Can I revoke a proof?">
  ...
  <Accordion title="Which wallets can sign in the wallet-link flow?">
  ...
  <Accordion title="Why was my statement rejected?">
