...
flowchart LR
    ...
    pay --> conv["FX conversion at the current<br/>rate − payin fee"]
...
|---|---|---|
| Chile | CLP | Announced bank transfer |
...
USDT at the current rate and credited net of the payin fee.

## 2. Examples by country

<Tabs>
<Tab title="Chile">

**Announced bank transfer**: announce the incoming deposit and share the
reference with the sender.
...
The response carries the `reference` that must travel in the transfer
description for automatic matching.
...
You can also use the **announced bank transfer**
(`method: "bank_transfer"`), sharing the reference in the PIX description.
