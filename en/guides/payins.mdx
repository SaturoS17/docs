...
flowchart LR
    ...
    announced["Announced transfer<br/>(CL, PE, MX, PY, BR)"] --> pay
...
payer will transfer (e.g. `"596000"`). The `reference` in the transfer
concept ensures the automatic match; amount+currency matching works as a
fallback.
</Note>
