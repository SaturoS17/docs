...
country. The amount converts from local currency to USDT at the current rate
and `usdt_amount + fee` is debited from your balance.
...
| 422 | (payout with `status: failed`) | The corridor rejected the data; the debit was already refunded â€” fix `beneficiary` and retry with a new key |
