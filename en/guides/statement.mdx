...
```json
{
  ...
  "crypto_withdrawals": [ { "…": "…" } ],
  "transfers": [ { "direction": "sent", "counterparty": "Ana Perez", "amount": "25.000000" } ],
...
1. **`summary`** — opening balance, inflows, outflows, closing balance,
   fees by service and the `balanced` flag.
2. **`breakdown`** — by product, by country (payouts and payins with local
   amount and USDT), by fiat currency and by month.
3. **Per-product detail** — payouts (beneficiary, rate and debit), payins
   (per mode), crypto (with `tx_id`), transfers (with counterparty) and
   service charges (with refunds).
4. **`movements`** — the raw ledger: every movement with its
...
- `balanced: true` confirms the identity holds against the ledger.
