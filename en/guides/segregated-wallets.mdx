...
icon: "wallet"
description: "On-chain wallets with their own balance for companies: create, import, receive, send, export the key and auto-forward — the balance lives on the blockchain, never in the ledger"
---

**Segregated wallets** are on-chain wallets **owned by your company
account**: their balance **is** the on-chain balance of the address, not a
virtual balance in the CBPay ledger. You can create as many as you want,
...
<Note>
Company accounts only. A person account gets `403 company_required`.
</Note>
...
```mermaid
flowchart LR
    create["POST /v1/wallets"] --> wallet["On-chain wallet<br/>(own balance)"]
    ...
    wallet --> export["POST /v1/wallets/{id}/export<br/>(private key)"]
```
...
(`GET /v1/wallets/{walletID}/sends/{sendID}`) includes `funding_sources`:
...
| HTTP | `error` | Fix |
|---|---|---|
| 403 | `company_required` | Segregated wallets are for company accounts only |
...
No limit. This is typical for segregating per client, project or business
unit. Each wallet can carry a `label` to tell them apart.
</Accordion>
</AccordionGroup>
