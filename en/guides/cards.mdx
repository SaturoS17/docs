...
Before the examples, the rule that organizes everything: **the cardholder
is verified ONCE per account**, on the first issuance.

| Situation | Do you send `cardholder`? |
|---|---|
| The account's **first card** (person, or company for itself) | **Yes, complete** — creates and verifies the holder at the issuer |
| **Subsequent cards** of the same account | No — the verified holder is reused (`physical` + `idempotency_key` is enough) |
| Company issuing **for an employee** (`cardholder.kind: "person"`) | **Yes, complete, always** — every designated person is a new holder |

The idempotency key is required in every case: a retry with the same key
returns the original card and never double-charges.

### First card (creates the holder)

<CodeGroup>

```bash Person — first card (full data)
...
```bash Company — for an employee (always full)
curl -X POST https://api.qbank.cl/platform/v1/cards \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
...
If they are missing or insufficient, issuance fails (`422 core_rejected`
or `409 cardholder_kyc_pending`), **the fee is refunded automatically**
and you can retry with corrected data.

On a company account's **first issuance for itself**, the `cardholder`
carries the corporate data instead of the personal one:
`kind_of_business` (catalog code), `legal_representation` and the
documents by URL `certificate_of_good_standing_url`,
`business_license_url`, `register_shareholder_url`, `id_shareholders_url`
and `address_verification_shareholders_url`.
</Warning>

### Subsequent cards (holder already verified)

Once the holder exists, later issuances for that account need no data:
...
<Note>
When issuing for a **designated person** (companies), identity documents
are required in that same call: the holder is verified immediately and the
card is issued along with it (`cardholder_kind: "person"` in the
response). The printed name uses `first_name` + `last_name` (22 characters
combined max). Remember the limit: person accounts can hold **1 virtual +
1 physical**; companies, unlimited.
</Note>
