...
**Your first card** creates and verifies your holder at the issuer, so it
carries your full data and identity documents by URL:
...
`salary_usd` is whole US dollars.
...
`cardholder.kind: "person"` with THAT person's full data — every
designation is a new holder, so their data and documents go **every
time**, on each card for a different person:
...
max) and the response carries `cardholder_kind: "person"`.
