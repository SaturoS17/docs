...
| 400 | `invalid_value` | Org setting `default_locale` is not `en`/`es`/`zh` or `""` | Platform-admin only; see [errors](/en/errors) |
...
Full catalog: [Errors](/en/errors). Profile fields: [Your profile](/en/guides/profile).

## FAQ

<AccordionGroup>
<Accordion title="Why is my existing account in Spanish after this release?">
...
<Accordion title="Which cookie should my checkout set?">
