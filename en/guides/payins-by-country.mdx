---
title: "Payins by country"
icon: "earth-americas"
sidebarTitle: "Country reference"
description: "Every collection mode with its real request and response: CL, PE, MX, VE, BO and BR"
---

Complete reference for every [payins](/en/guides/payins) corridor: each
country's collection mode with the full request and the response you get
back. Rates are illustrative — your real `payin_rate` lives in
`GET /v1/rates`.
...
The `reference` must travel in the transfer description for the automatic
match; amount+currency is used as a fallback matcher.
...
image (`charge.qr_image`).

You can also use the **announced bank transfer**, sharing the reference in
the PIX description:
...
<Note>
The live catalog is `GET /v1/payins/methods` — availability per country
may vary. Whatever the mode, the credit always lands the same way:
conversion at your `payin_rate` minus the fixed fee, a `payin_credit`
entry in your movements and the `payin_credited` webhook.
</Note>
