...
```mermaid
flowchart LR
    reg["1. POST third-parties<br/>(third party identity)"] --> docs["2. Documents + submit<br/>(third party KYC)"]
    docs --> approved{"approved?"}
    approved -->|"Yes"| accts["3. POST /accounts<br/>(accounts in their name)"]
    approved -->|"No"| fix["Fix and resubmit"]
```

### Registering the third party

The identity goes complete and explicit (`type` INDIVIDUAL or COMPANY is
required — it is ANOTHER person, nothing is copied from your company). The
banking profile fee is charged (refunded if the registration fails):
...
| 400 | `invalid_type` | The third party `type` must be INDIVIDUAL or COMPANY |
| 404 | `not_found` | The third party does not exist or does not belong to your account |
