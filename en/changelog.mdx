...
- **PIX QR payout in Brazil (BR/BRL)**: the two-step flow
  ...
  **static** Brazilian PIX QRs (including the "copia e cola" code) — send
  ...
  payout. Fixed-amount QRs require an exact match (or omit `amount`);
  open-amount QRs require `amount`. Dynamic QRs (PSP URL payload) answer
  `400` — use the `pix` method with the beneficiary's key. Available in the
