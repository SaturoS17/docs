---
title: "Authentication"
description: "JWT sessions, API keys and access levels"
---

Every call (except register and login) requires a credential in the
`Authorization` header:

```
Authorization: Bearer <token>
```

`X-API-Key: <token>` is accepted as an alternative header.

## Credential types

<Tabs>
<Tab title="JWT session (users with login)">

Obtained from `POST /v1/auth/register` or `POST /v1/auth/login`, valid for
**24 hours**. Meant for apps where users sign in.

```bash
curl -X POST https://exchange.qbank.cl/platform/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{ "org": "cbpay", "email": "ana@example.com", "password": "…" }'
```

```json
{
  "access_token": "eyJ…",
  "expires_at": "2026-07-08T00:00:00Z",
  "account_id": "…",
  "role": "owner"
}
```

Company accounts can have multiple **members** with `owner`, `operator` or
`viewer` roles (`POST /v1/members`).

</Tab>
<Tab title="API key (server to server)">

Format `pk_<key_id>.<secret>`. Never expires and is not tied to a session.
Issue one with:

```bash
curl -X POST https://exchange.qbank.cl/platform/v1/api-keys \
  -H "Authorization: Bearer <session-token>" \
  -H "Content-Type: application/json" \
  -d '{ "label": "production-backend" }'
```

```json
{
  "api_key_id": "…",
  "key_id": "a1b2c3d4e5f60718",
  "token": "pk_a1b2c3d4e5f60718.XXXXXXXX…",
  "note": "store this token now; it cannot be retrieved again"
}
```

<Warning>
The plaintext token is shown **exactly once**. Only a hash is stored server
side — if you lose it, issue a new key.
</Warning>

</Tab>
</Tabs>

## Access levels

| Level | Credential | What it can do |
|---|---|---|
| **Account** | Session JWT or account API key | Operate its own account: balances, payouts, payins, transfers, crypto, KYC, own webhooks |
| **Organization admin** | Organization API key | Manage every account of the organization: create them, configure fees, adjustments, revenue, assign payins |

Administration endpoints live in the [Admin](/en/admin/introduction) tab.
Calling an account endpoint with an admin credential returns
`403 account_required`, and vice versa `403 org_admin_required`.

## Best practices

- Store API keys in a secrets manager; never in code or in the browser.
- Use one key per environment/service (descriptive `label`) so you can
  rotate without downtime.
- JWT sessions are for front-ends; automated processes should always use
  API keys.
