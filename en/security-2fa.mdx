...
- If the phone is linked for the first time (or changed without
  verification), protected actions stay locked for **24 hours**
  (`403 phone_binding_cooldown`): the anti session-hijacking window. A
  phone set by your operator has no cooldown.
...
| 403 | `phone_binding_cooldown` | Phone linked less than 24 h ago without verification | Wait out the cooldown or ask your operator to set the number |
