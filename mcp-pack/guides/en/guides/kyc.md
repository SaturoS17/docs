...
**verification report** generated by the platform: verified identity, decision
lifecycle, documents with OCR, liveness check and AML screening (sanctions /
PEP / adverse media), with an integrity hash and a public verification code.
Two formats (`?format=pdf|json`, default `pdf`) and three languages
(`?lang=en|es|zh`, default `en`). It is free: it is a read of a verification
