...
(RFC 3339) in the create, get and list responses, and the
`payin_settlement_scheduled` webhook fires exactly once at confirmation
(idempotent) with the scheduled amounts. Everything else happens when that
time is reached:
...
account. Your integration gains one signal — `payin_settlement_scheduled`
at confirmation — and reads `settle_at`; the credit, `payin_credited` and
final states are unchanged, just delivered at settlement.
## Banking rail fees
