...
| `payin_settlement_scheduled` | A card payin was paid and its credit was scheduled for a future `settle_at` (org-configured settlement delay). Emitted exactly once at payment confirmation; the balance credits when due and `payin_credited` fires then |
