...
| `payin_settlement_scheduled` | Un cobro con tarjeta quedó pagado y su acreditación se programó para un `settle_at` futuro (espera de settlement configurada por la org). Se emite exactamente una vez al confirmarse el pago; el saldo se acredita al vencimiento y ahí se emite `payin_credited` |
