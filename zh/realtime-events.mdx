...
用 [Webhook](/zh/webhooks) 处理那些即使用户关闭浏览器也必须完成的逻辑。两个通道传递
...
（字段与 [`GET /v1/balances`](/zh/guides/analytics) 相同）；组织管理员还会得到运营
`health` 计数。
...
均不可变，会在各自的端点和[对账单](/zh/guides/statement)中按监管要求的期限继续保留。
</Warning>
...
| HTTP | 代码 | 解决方法 |
|---|---|---|
| 400 | `invalid_event_type` | 使用 [Webhook 目录](/zh/webhooks)中的类型。 |
