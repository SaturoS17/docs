...
- 如果手机号是首次绑定（或未经验证即被更改），受保护的操作将锁定 **24 小时**（`403 phone_binding_cooldown`）：这是防会话劫持的窗口期。由运营方设置的手机号没有冷却期。
...
| 403 | `phone_binding_cooldown` | 手机号绑定未满 24 小时且未经验证 | 等待冷却期结束，或请您的运营方设置号码 |
