...
- **所有有可用代收走廊的国家**——选择自己的国家后即可看到可用方式
  （二维码、银行卡、银行转账、托管支付页）及即时报出的当地金额。
- **4 种加密货币选项**（TRON 上的 USDT、以太坊上的 USDT/USDC 以及
  BTC），每笔收款专属充值地址，并附带**可扫描二维码**，兼容外部钱包
  （Trust Wallet、MetaMask、Binance 等）。
- **使用 CBPay 应用直接支付**：商户别名和二维码始终可见——用应用扫码
...
- **加密货币（每笔收款一个钱包）**：选择币种后生成专属地址及其
  `qr_payload` 和 `qr_png_base64`——BTC 使用 BIP-21 URI
  （`bitcoin:<address>?amount=…`，自动填充金额）；TRON/ETH 的二维码为
  裸地址（兼容性最高），金额显示在旁边可复制。若支付资产与
...
- `GET {checkout_url}/state`——链接状态：`status`、`paid_method`、
  ...
  `conversion_status`。
- `GET {checkout_url}/quote`——选择前的报价：`countries`（含各国货币
  与方式的目录）、`crypto`（各币对的参考应付额）和 `cbpay`（别名 +
  各资产应付额）。带 `?country=XX` 时额外返回该国当地金额的
  `country_quote`。
- `POST {checkout_url}/methods/{method}`——物化所选支付选项。法币方式
  要求 `?country=XX`；加密货币使用 `crypto:<chain>:<asset>`
  （如 `crypto:tron:usdt`），不带国家。对同一国家+方式重复 POST 返回
  **同一个**物化结果。
