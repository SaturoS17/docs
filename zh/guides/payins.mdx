...
如果你不想替客户预先选定收款方式，可以创建一个**通用收款链接**：一次
`POST /v1/payins`（`method: "checkout"`）即可返回一个带品牌的公开 URL，
付款人在页面上自行选择支付方式——二维码、银行卡、银行转账或**加密货币**
（TRON 上的 USDT、以太坊上的 USDT/USDC 以及 BTC，且会为该笔收款生成专属
充值地址）。

```mermaid
sequenceDiagram
    participant I as 你的集成
...
- **法币方式**：页面按目录（`GET /v1/payins/methods`）提供该收款国家和
  币种可用的方式——二维码、银行卡、托管支付页或带 `reference` 的银行
  转账通知。入账按正常 payin 汇率与手续费结算。
- **加密货币（每笔收款一个钱包）**：选择币种后会生成**该笔收款专属的
  充值地址**并给出精确报价（USDT/USDC 按当时美元等值；BTC 报价每 15
  分钟刷新）。部分支付会累加，页面显示剩余金额；达到目标后收款即完成。
  入账为收到的资产扣除你的 funding 手续费。

链接规则：

- **一个链接 = 一笔收款**：最先完成支付的方式生效，其余选项随之失效
  （在无人付款前，选择某个方式不会锁定其他方式）。
...
- 使用相同 `idempotency_key` 重试会返回**同一个链接**（URL 不变），
  绝不会开启第二笔收款。
- 状态可通过 `GET {checkout_url}/state` 免认证查询（JSON，含 `status`、
  `paid_method` 及加密支付进度）——如果你想在同一链接上渲染自己的支付
  页面会很有用。
- 收款完成后你会收到正常的 `payin_credited`；若以加密货币支付，事件会
  附带 `settled_via`（如 `crypto:tron:usdt`）和链上实收金额
  `crypto_amount`。
...
| 422 | `method_unavailable` | 该方式在此收款走廊不可用 |
