...
title: "隔离钱包"
icon: "wallet"
description: "面向企业、拥有独立余额的链上钱包：创建、导入、收款、发送、导出私钥与自动转发——余额存在于区块链上，绝不进入账本"
---

**隔离钱包**是**归属于您的企业账户**的链上钱包：其余额**就是**该
地址的链上余额，而非 CBPay 账本中的虚拟余额。您可以创建任意数量
的钱包，接收并**直接从每个钱包发送加密货币**，通过私钥**导入**
外部钱包，并可随时**导出**私钥（共享托管）。它们非常适合按客户、
按项目或按业务单元隔离资金，并完全掌控私钥。

<Note>
仅限企业账户。个人账户会收到 `403 company_required`。
</Note>
...
| HTTP | `error` | 解决方式 |
|---|---|---|
| 403 | `company_required` | 隔离钱包仅面向企业账户 |
...
没有限制。按客户、项目或业务单元进行隔离是典型用法。每个钱包都
可以携带 `label` 加以区分。
</Accordion>
</AccordionGroup>
