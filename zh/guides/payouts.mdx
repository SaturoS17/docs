...
在玻利维亚（本地互操作 QR 标准），您还可以分两步**支付收款二维码**：
扫码与确认。扫码**免费**；只在确认时收费，与普通出金完全相同（您的
汇率 + 固定费用）。如未提供 `country`/`currency`，默认按玻利维亚
（BOB）处理。支付巴西 PIX 二维码即将推出（在此之前请使用按密钥或到
账户的 `pix`）。
...
- 结果为**同步**返回：响应中已包含最终状态（`completed`，或 `failed`
  并自动退款）—— 无需等待。
- 已扫描的二维码只能支付一次；使用相同 `idempotency_key` 重试会返回
  原始出金。
- 巴西 PIX 二维码出金上线后，`qr_payload` 将接受二维码内容或 PIX
  "copia e cola" 代码；同一流程同时覆盖静态和动态二维码。
