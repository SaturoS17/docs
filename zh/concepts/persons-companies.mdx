...
| 每条网络的**加密钱包**数量 | **1 个**（创建第二个时返回 `422 wallet_limit_reached`） | **不限**（用 `label` 加以区分） |
...
- 验证：通过 `POST /v1/me/verification/link` 获取您的 KYC 链接并
  完成向导 — 在通过之前您只能进行入金
  （[指南](/zh/guides/kyc)）。
- 加密货币：**每条网络一个钱包**（TRON 和 ETH）。需要更多地址？
  那属于企业账户的使用场景。
...
- 验证：通过 `POST /v1/me/verification/link` 获取您的 KYB 链接并
  ...
  （[指南](/zh/guides/kyc)）。
- 加密货币：每条网络**不限数量的钱包**（可按分支机构、按产品、
  按供应商各建一个……），并使用描述性的 `label`。
...
| `422 wallet_limit_reached` | 个人账户在同一网络上尝试创建第二个钱包 |
