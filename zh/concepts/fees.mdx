...
| `wallet_import` | 每次导入固定费用 | 将外部钱包导入为[隔离钱包](/zh/guides/segregated-wallets)时（`POST /v1/wallets/import`） |
| `wallet_export` | 每次导出固定费用 | 导出隔离钱包私钥时（`POST /v1/wallets/{id}/export`） |
| `wallet_send` | 每次发送固定费用 | 从隔离钱包发起链上转账时（`POST /v1/wallets/{id}/sends`）；网络 gas 由客户承担 |
