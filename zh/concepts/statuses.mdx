...
- `credited` — 已收到付款，按您的 `payin_rate` 换算并入账。若配置了
  ...
  `settled_at: null`（见
  [银行卡收款结算延迟](/zh/concepts/fees#银行卡收款结算延迟)）。
