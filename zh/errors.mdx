...
| 422 | `verification_required` | [QR POS](/zh/guides/qr-pos)：用商户已批准的第三方 KYC/KYB 的 `verification_id` 注册商户 |
| 422 | `merchant_disabled` | [QR POS](/zh/guides/qr-pos) 商户已停用；先重新启用再生成收款 |
| 422 | `nothing_received` | [QR POS](/zh/guides/qr-pos) 收款未收到任何链上支付：无可退款 |
| 422 | `refund_exceeds_received` | 退款超过 [QR POS](/zh/guides/qr-pos) 收款已收金额减去已退金额 |
| 400 | `to_address_required` | [QR POS](/zh/guides/qr-pos) 退款（及加密货币提现）必须显式提供目标地址 |
