...
- **巴西 PIX 二维码出金（BR/BRL）**：两步流程
  ...
  **静态** PIX 二维码（含 "copia e cola" 代码）——传 `country: "BR"` 与
  ...
  密钥与金额；确认步骤通过 PIX 付款，计费与常规 payout 完全相同。固定金额
  二维码要求金额完全一致（或省略 `amount`）；开放金额二维码必须提供
  `amount`。动态二维码（载荷为 PSP URL）返回 `400`——请改用 `pix` 方式并
  提供收款人的密钥。测试环境已提供示例二维码与魔法值（`.99` 金额失败）——
