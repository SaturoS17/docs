...
Las operaciones fiat se convierten a USDT con la tasa vigente del momento
(`GET /v1/rates`, base USD). La conversión redondea **hacia arriba** en el
débito (nunca se te cobra de menos… ni se debita de más de 1 micro-USDT de
diferencia).
...
| `compliance_fee` / `compliance_refund` | Cargo por servicio KYC / reembolso |
| `adjustment` | Ajuste manual del administrador (auditado) |
